...
async function getQuotes() {
  let url = `https://reddirani8496.github.io/BhagavadGitaData/quotations.json`;
...
quote.addEventListener("click", () => {
  ...
  navigator.clipboard.writeText(quoteText); // to copy the text
});
  ...
  if (arrowkeys.includes(e.key)) {
    ...
    if (e.key == "g" || e.key == "G") {
      ...
      document.body.innerHTML = `
        ...
        <h1 class="glow" style="font-size:290%">Surprise unlocked, wait for a moment!</h1> 
        </div>`;
      setTimeout(() => {
  ...
  $("#download-btn").on("click", function () {
    html2canvas(element, {
      onrendered: function (canvas) {
        ...
        $("#download-btn").attr("download", "image.jpg").attr("href", newData);
      },
    });
  });
});
